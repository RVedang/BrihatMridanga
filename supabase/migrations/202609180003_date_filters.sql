-- Inclusive temple-local distribution dates, independent of entry submission time.
create function public.public_scores_range(start_date date, end_date date, selected_campaign uuid default null)
returns table(temple_id uuid,temple_name text,country text,books bigint,known_points numeric,incomplete_reports bigint)
...
revoke all on function public.public_scores_range(date,date,uuid) from public;
grant execute on function public.public_scores_range(date,date,uuid) to anon,authenticated;
